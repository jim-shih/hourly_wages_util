...
class ShiftDataReader:
    ...
    def __init__(self, _shift_type_file: Path, _shift_file: Path, _holiday_file: Path):
...
class ShiftCalculator:
    ...
    def _calculate_overtime_pay_rate_2(self, overtime_hours: float) -> float:
        ...
        base_pay = self.base_hours * self.hourly_rate
        overtime_pay_1 = (
            self.overtime_hours_limit * self.hourly_rate * self.overtime_rate_1
        )
        overtime_pay_2 = (
            (overtime_hours - self.overtime_hours_limit)
            * self.hourly_rate
            * self.overtime_rate_2
        )
    ...
    def calculate_total_working_hours_and_wage(self) -> Tuple[float, float]:
        ...
        total_working_hours = round(total_working_time.total_seconds() / 3600, 2)
    ...
    def __str__(self) -> str:
        (
            total_working_hours,
            total_salary,
        ) = self.calculate_total_working_hours_and_wage()
